...
export async function createRex(scene,onProgress){
 ...
 let reaction=0,lastVocal={jaw:0,energy:0};const head=find('head_');
 ...
 return{actor,meshes,skin,damage,bones,head,gait,death,gaze,get vocal(){return lastVocal;},hit(){reaction=.16;},reset(){damage.reset();death.reset();gaze.reset();reaction=0;gait.reset();actor.visible=true;},drainMotionEvents(){return [...gait.drainFootfalls(),...death.drainEvents()];},mouthPosition(){mouthIndices.forEach((index,i)=>{skin.getVertexPosition(index,mouthPoints[i]);skin.localToWorld(mouthPoints[i]);});return{upper:mouthPoints[0].clone(),lower:mouthPoints[1].clone(),center:mouthPoints[0].clone().lerp(mouthPoints[1],.5)};},update(dt,state,time,roadSpeed=10,vocal=null){
