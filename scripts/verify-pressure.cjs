...
(async()=>{
 ...
 try{
  for(const [name,width,height,touch,view]of [['first',1600,1000,false,'first'],['third',1600,1000,false,'third'],['phone',390,844,true,'first'],['small-phone',390,680,true,'first'],['landscape',844,390,true,'first'],['phone-third',390,844,true,'third']]){
   ...
   const sites=await p.evaluate(()=>{
    ...
    for(const t of r.targets.targets){
     ...
     const controls=['#mission-clock','#challenge-card','#touch-fire','#touch-reload'].map(s=>{const e=document.querySelector(s),b=e.getBoundingClientRect();return{selector:s,x:b.x,y:b.y,right:b.right,bottom:b.bottom,shown:getComputedStyle(e).display!=='none'&&b.width>0&&b.height>0};});
   ...
   for(const s of sites){assert.equal(s.visible,true,`${name} ${s.name}: hidden anchor`);assert.equal(s.hit,true);assert.equal(s.progress,1,`${name} ${s.id}: target ray rejected`);assert.deepEqual(s.layoutIssues,[],`${name} ${s.id}: roaring variant covered by HUD`);for(const b of s.controls.filter(b=>b.shown)){const t=s.point;assert.ok(t.x+t.radius<b.x||t.x-t.radius>b.right||t.y+t.radius<b.y||t.y-t.radius>b.bottom,`${name} ${s.id}: target covered by ${b.selector}`);}}
