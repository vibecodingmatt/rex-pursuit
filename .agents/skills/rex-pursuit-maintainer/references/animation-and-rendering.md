...
- **Shot compies tumble about their body centre.** The mesh origin is at the feet; spinning about it swung the body around the feet. While dead, `c.p` is the centre and `write()` offsets the instance by the rotated centre. Ground contact uses a small hull of sample points (feet, belly, back, head, tail tip, flanks), so any orientation lands without sinking. The death pose lives in the gait shader (`aPose.w`), so it costs no extra draw.
- **The detour's crossers must be visible from the gun.** In first person the gun hides the middle of the road nearer than about 14 m, and the verge hides anything beyond |x| ≈ 5. They start 7–11 m back, inside the verge on her side, and run slanted toward the Jeep so the road carries them away at only 3–5 m/s while they cross. Farther out they were 15 px specks.
- **Targets must be where the gun can see them.** From the gunner's seat the canopy leaves the sky open only over the road, so Pteranodon crossing from the forest were hidden for most of their flight; flying down the corridor from far behind the Rex they stay in view for about ten seconds. Perched Dimorphodon read only within about 35 m, and folded wings made a sliver; they perch with the wings half open, and the Rex striding past flushes them in front of the gun. The Gallimimus herd crosses behind the Rex (z 22–32), where it cannot overlap her or steal shots aimed at her, and slants toward the Jeep so it holds its distance while crossing. Check with `art/review/wildlife/sky-film.cjs` and `roost-probe.cjs` (screen positions of each animal) before trusting a projection count: most "in view" roosts were 50–100 m away and invisible.
- **Frame rate in checks:** headless Chrome here runs animation frames far above 60 Hz, so a loop of N frames is much less game time than it looks. Time-limit loops that wait for game events (`verify-wildlife.cjs` `shootAt`).
- **Rear-up: weight by height, not only the limb mask.** Rotating the body about the hips while the hind legs stay put tears the groin if the blend follows `aux.w`: that mask is only about 0.5 near the claws, and it switches over a hand's width at the thigh. Planting by height (fully below 1.2 m, none above 4.4 m), with a fore/hind split that widens with height, bends the belly over metres. `art/review/wildlife/rear-probe.mjs` replays the pose on the baked mesh and reports hind-foot drift, forefoot lift, tail clearance and stretched edges. Straight up, the neck lined up with the trunks in the storm; the neck counter-pitch makes her reach forward over the open road corridor. Keep `posePoint()` in step with the shader stage: the call's sound position and the hit capsules use it.
- **Nothing appears or vanishes in view.** Safari crossers start out of the camera's frustum or behind the understory belt (|x| ≥ 16.5; 19 for the quadrupeds) and keep running until they leave the view; Safari pterosaurs arrive out of the corridor haze; a Dimorphodon perches only on a real roost and is tied to its chunk (`c.home`), so a chunk wrap, reset or ford swap removes it rather than leaving it clinging to air. `art/review/bugs3/entry.cjs`, `spawns.cjs` and `roosts.cjs` check these.
- Gait and wing shaders take a phase wrapped to 0..1 on the CPU (`aPose.x`, `aFly.x`), never a clock (see below).
- **Safari sculpts (`scripts/build-safari.mjs`).** Author each head in its own frame with `placer()` (origin at the neck joint, z along the skull, a pitch for posture) so posture never distorts anatomy; carve sockets and nostrils with `carve()`; build frills, crests and plates as `sheet()`/`plate()` (they never go thinner than 0.6 voxel, or the Low tier breaks them up). A face that tapers smoothly out of the neck reads as a trunk or a sausage: make the skull deeper than the neck end, with a throat between, and end it in a distinct beak or snout tip. Positions are 16-bit inside each model's bounds and wrap outside them (a Parasaurolophus crest poking out smeared a slab from the ground to the sky), so the bake now throws when a surface reaches its bounds. The hide rides in the spare bytes (colour alpha: fine scales; normal w: gloss +, large scales −). The shader's scale bump is the analytic Worley gradient turned into view space: screen-space `dFdx` bumps shade in 2×2-pixel blocks up close. Scattered raised tubercles read as warts; a pavement of large low scales reads as hide. Review with `art/review/safari3/look.cjs` (skinned and clay sheets, title views), `kill.cjs` and `perf.cjs`; details in the handoff.
...
- **Night foliage turned pale blue frost, and daytime leaves grey-white.** Zeroing each light's intensity in turn showed that only the rim light (the moon, at night) does it. Don't toggle `visible`: that renumbers the light slots and misleads. It has no shadow map, so it lit every leaf in the forest alike. A quarter of it at night still read as frost to the user (2026-09-26). Leaves now leave directional light 1, the rim, out of three's light loop entirely (`LEAF_LIGHTS` in `foliage.js`). Keep the rim the second directional light: the sun sorts first as the only shadow caster. The Rex, road and trunks keep it.
