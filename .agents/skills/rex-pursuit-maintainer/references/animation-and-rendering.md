...
- **Smooth blends inflate.** A smooth minimum swells the surface by up to k/4 wherever two blended primitives meet at similar depth. Chains of segments (neck, tail) must join with a near-zero blend, or each joint becomes a ring (the user saw lumps down the neck). Give depth with one continuous tapering form (the throat fairing), not separate bulges.
- **The body matches a reference, not guesswork.** The user supplied the Jurassic World brachiosaur (`art/review/drop6/ref-jw-brachio.png`, kept in ignored art/review; copy it somewhere safe before cleaning). Guessed proportions drew three rounds of "silly" feedback. The torso and tail are now one `loft()`: explicit dorsal-line, ventral-line and width curves along z. Edit those keys to change the silhouette, and check it with `brachio-measure.cjs` (numeric side profile) and `brachio-studio.cjs ref2` (a low, close three-quarter like the photo; a close camera near the head exaggerates the neck). Target ratios for a 14.9 m head: withers .43, belly .15, a neck about 0.8 m through at 10 m up, legs 1–1.2 m thick, and a tail thick at the root that runs out level and droops.
- **Keep the back line one straight slope** from the withers over the hips into the tail, set by a thin dorsal ridge. Round torso lobes stacked along the back left a saddle before the tail (the user caught it). Nothing, thigh muscle included, may rise above that line. Keep the belly high enough that the legs read as long pillars; a deep, low belly makes a sauropod look stumpy.
