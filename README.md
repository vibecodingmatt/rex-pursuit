...
- `public/models/brachio.bin`, `brachio-low.bin` — the brachiosaur, generated by `npm run art:brachio` (`scripts/build-brachio.mjs`): a Giraffatitan-proportioned body sculpted as a signed distance field, meshed, and baked with occlusion, crease cavity, body regions and a spine coordinate for the neck and tail. The low version is for the Low tier.
