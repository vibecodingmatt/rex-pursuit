...
The seven large ground species (Velociraptor, Dilophosaurus, Pachycephalosaurus, Parasaurolophus, Triceratops, Stegosaurus and the Gallimimus, which also crosses in the chase) are original sculpts baked from signed distance fields into High and Low meshes (`npm run art:safari` rebuilds `public/models/safari-runners.bin`, 5.4 MB, about 3.2 MB gzipped as served). Heads are modelled in their own frame with carved eye sockets and nostrils; frills, crests and plates are thin curved sheets. The bake paints the hide (mottling, bands, eyes with pupils and lids, lip lines and teeth) and marks where fine scales, large scales and glossy horn, beak and eyes belong; on High the shader turns those marks into scale relief. They share the critter rig: bipeds swing their legs about the hips; the quadrupeds trot on diagonal pairs and roll onto their side when they die. Hit spheres run nose to tail, so a nine-metre animal can be hit anywhere along its body. Low quality reduces shadows, mesh detail and environment effects without reducing scoring opportunities. Safari skips the unused Rex rig, cinematic work and scenic brachiosaur, and costs less per frame than the chase. `npm run test:safari` covers its rules and focused browser flow; pure rules also run in CI through `test:logic`.
